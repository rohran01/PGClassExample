...
$(document).ready(function(){
    $("#taskForm").submit(function(event){
        ...
        formData += "&complete=false";                          //What is this "complete" business???
        $.ajax({
            type: "POST",
            data: formData,
            url: "/api/todos",
...
function appendTasks(){
    ...
    for(var i = 0 ; i < taskData.length ; i ++){
        ...
        $el.data("id", taskData[i].id);                                             //Where are these data points being attached?
        $el.data("complete", taskData[i].complete);
        if(taskData[i].complete){
            $el.css("text-decoration", "line-through");                            //what is this doing?
        }
